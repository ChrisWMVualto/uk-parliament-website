    ...
    public partial class SearchController : Controller
    {
        ...
        public virtual ActionResult Index(string keywords, int? memberId, string member, string house, string business, string start, string end, int page = 1)
        {
            ...
            if (ModelState.IsValid)
            {
                if(!firstSearchLoad)
                {
                    searchModel.SearchResult = _searchService.Search(keyWordsOrMember, null, house, business, fromDate.Date, toDate.AddDays(1).AddSeconds(1), page, isMemberKeywordSearch, false);
                }
            }
